...
pub fn verify_alignment_cigar(
        ...
        match op {
            CigarOp::Match(len) => {
                ...
                if q_pos + len > oriented_query.len() {
                    errors.push(format!(
                        "Op {op_idx}: Match({len}) exceeds query bounds at pos {q_pos}"
                    ));
                    break;
                }
                if t_pos + len > target_seq.len() {
                    errors.push(format!(
                        "Op {op_idx}: Match({len}) exceeds target bounds at pos {t_pos}"
                    ));
                    break;
                }
            ...
            CigarOp::Mismatch(len) => {
                ...
                if q_pos + len > oriented_query.len() {
                    errors.push(format!(
                        "Op {op_idx}: Mismatch({len}) exceeds query bounds at pos {q_pos}"
                    ));
                    break;
                }
                if t_pos + len > target_seq.len() {
                    errors.push(format!(
                        "Op {op_idx}: Mismatch({len}) exceeds target bounds at pos {t_pos}"
                    ));
                    break;
                }
            ...
            CigarOp::Insertion(len) => {
                ...
                if q_pos + len > oriented_query.len() {
                    errors.push(format!(
                        "Op {op_idx}: Insertion({len}) exceeds query bounds at pos {q_pos}"
                    ));
                    break;
                }
            ...
            CigarOp::Deletion(len) => {
                ...
                if t_pos + len > target_seq.len() {
                    errors.push(format!(
                        "Op {op_idx}: Deletion({len}) exceeds target bounds at pos {t_pos}"
                    ));
                    break;
                }
    ...
    if q_pos != oriented_query.len() {
        errors.push(format!(
            "CIGAR only consumed {q_pos}/{} query bases",
            oriented_query.len()
        ));
    }
    if t_pos != target_seq.len() {
        errors.push(format!(
            "CIGAR only consumed {t_pos}/{} target bases",
            target_seq.len()
        ));
    }
