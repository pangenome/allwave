...
fn find_microsatellites(sequence: &[u8], min_unit: usize, max_unit: usize, min_repeats: usize) -> Vec<(usize, usize, Vec<u8>)> {
    ...
    for (start, length, unit) in microsatellites {
        if rng.gen::<f64>() < config.microsatellite_rate {
            ...
            if adjusted_start < mutated.len() && adjusted_end <= mutated.len() {
                ...
                let original_str = String::from_utf8(mutated[adjusted_start..adjusted_end].to_vec()).unwrap();
        ...
        if rng.gen::<f64>() < config.cnv_rate {
            ...
            if is_duplication {
                ...
                if adjusted_pos + cnv_size <= mutated.len() {
                    ...
                    for _ in 1..copies {
                        mutated.splice(adjusted_pos + cnv_size..adjusted_pos + cnv_size, segment.clone());
...
pub fn create_test_case(name: &str, seq_config: &SequenceConfig, mut_config: &MutationConfig) -> TestCase {
